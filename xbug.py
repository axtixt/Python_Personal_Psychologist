...
class TwitterExtractor:
    ...
    def fetch_tweets(self, page_url, start_date, end_date):
        ...
        while True:
            try:
                ...
                # 日期解析与验证
                try:
                    tweet_date = datetime.strptime(row["date"], "%Y-%m-%d")
                except ValueError:
                    try:
                        tweet_date = datetime.strptime(row["date"], "%d/%m/%Y")
                    except:
                        logger.warning(f"无法解析日期格式: {row['date']}")
                ...
                full_tweet_info = {
                    ...
                    "text": row["text"],          # 保留完整文本
                    "url": row["url"],            # 唯一标识
                    "language": row["lang"],
                    "likes": row["num_like"],
                    "retweets": row["num_retweet"],
                    "replies": row["num_reply"],
                    "is_retweet": row["is_retweet"],
                    "media_type": row["media_type"],
                    "mentioned_urls": row["mentioned_urls"],
                    "image_urls": row["images_urls"]
                }
                
                tweet_data.append(full_tweet_info)
                self._delete_first_tweet()
    
                time.sleep(max(0.5, min(2.0, abs(random.gauss(1.0, 0.3)))))
    
            except Exception as e:  # 现在这里的except可以正确对齐try
                logger.error(f"处理推文时发生错误: {str(e)}")
                self.driver.save_screenshot("error_screenshot.png")
                break
    
        # 数据去重处理
        unique_data = self._remove_duplicates(tweet_data)
    ...
    def _remove_duplicates(self, data):  # 修正缩进到类内部
        """基于URL去重"""
        seen = set()
        return [item for item in data if not (item["url"] in seen or seen.add(item["url"]))]
...
if __name__ == "__main__":
    ...
    tweet_data = scraper.fetch_tweets(
